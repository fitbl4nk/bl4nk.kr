...
- main의 `new`가 `/home/cg_casino/voucher`에 저장할 파일명일 것
- main의 `old`가 길이가 32이고 `/proc/self/environ` 파일을 가리킬 것
...
`**envp`(`0x7fffffffdf28`) -> `*envp`(`0x7fffffffe0a8`) -> `first env`(`0x7fffffffe276`)` 형식으로 구성되어있다.
