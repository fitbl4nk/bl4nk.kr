...
[taxonomies]
tags = ["ctf", "pwnable", "uaf", "fastbin reverse into tcache", "partial overwrite", "arbitrary write", "unsorted bin", "fsop", "stdout"]
+++
...
각각 전역변수에 선언된 `void *ptr_4060[32]`, `int size_4160[32]`에 저장돤다.

한편 `edit_1523()`에서는 `index`를 입력받아 `ptr_4060[index]`에 저장된 chunk의 내용을 수정할 수 있다.
...
이후 `size_4160[index]`는 0으로 초기화하지만 `ptr_4060[index]`는 초기화하지 않으므로 UAF 취약점이 발생한다.
...
위 자료들에서는 victim chunk를 해제하고 값을 쓸 수 있는 상황을 가정했지만 이 문제에서는 `size_4160[index]`가 0이면 `edit`이 불가능하므로 fastbin dup 상황을 추가로 만들어줘야 한다.

따라서 exploit 시나리오는 다음과 같다.

1. Fastbin 범위의 chunk를 `7`개 해제하여 tcache를 꽉 채움
2. UAF를 이용하여 fastbin dup 생성
...
이렇게 `refund`를 7번 실행해서 tcache를 꽉 채우면 이후 chunk들을 fastbin으로 보내진다.
...
이후 tcache를 비우기 위해 `buy`를 7번 실행하고 한번 더 `buy`를 실행하면 8번째 chunk가 반환된다.
...
잘 보면 tacahe list의 마지막에 `0x555555559640`이 오게 되는데 `size`가 0이다.
하지만 tcache에서 할당 시 `size` 검증을 하지 않아서 조작된 `next_chunk`의 할당이 이루어진다.
...
위 payload처럼 3번째 `buy`를 할 때 partial overwrite를 한 주소가 반환되며, 이를 이용해 heap에 저장된 값을 변경할 수 있다.

### Unsorted Bin Attack
이후 단계를 진행하기에는 바이너리에 출력하는 부분이 하나도 없어서 leak이 불가능했다.
지금 가지고 있는 것을 생각해보면 주소를 몰라서 그렇지 `next_chunk`을 조작해 AAW가 가능한 상황이다.
...
이제 chunk size를 overwrite하기 위해 fastbin reverse into tcache 기법을 이용한다.
...
따라서 fastbin reverse into tcache 기법을 한번 더 사용해서 해당 주소를 반환받는다.
...
`0x7ffff7fbfbe0`와 `0x7ffff7fc06a0`는 ASLR이 없을 때는 주소값이 3바이트 차이가 나지만 ASLR이 켜져있을 때는 확률적으로 2바이트만 차이가 나므로 partial overwrite를 했을 때 1/16 확률로 exploit이 가능하다.
...
이 `stdout` 구조체를 이용해서 AAR이 가능하고 바이너리에서 `flag`를 읽은 후 heap 메모리에 저장하므로 이제 heap 주소만 있으면 `flag`를 획득할 수 있다.
