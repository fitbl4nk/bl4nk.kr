...
입력한 문자열은 다르기 때문에 `strcmp(movie_ptr->name, src)` 검사는 통과하게 되고, hash한 값은 같기 때문에 `fd = fopen(src, "wb")`에서 같은 파일을 열게 된다.
...
### Arbitrary Free
이제 leak된 값들을 바탕으로 arbitrary free를 트리거해야하는데, 사실상 같은 취약점을 사용하지만 충돌쌍을 재활용하는 것이 어려우므로 새로운 충돌쌍을 사용했다.
